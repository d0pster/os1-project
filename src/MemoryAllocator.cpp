...
void MemoryAllocator::init () {
    ...
    MemoryAllocator::freeSegHead = (segDesc *) HEAP_START_ADDR;
    freeSegHead->size = ((uint64) HEAP_END_ADDR - (uint64) HEAP_START_ADDR) / MEM_BLOCK_SIZE;
}
...
void *MemoryAllocator::mem_alloc (size_t size) {
    ...
    for (; curr != nullptr && size > curr->size; prev = curr, curr = curr->next);
